...
def createLegends():
    ...
    legend_constants[
        39] = "buffon in component calcium_buffers_in_the_myoplasm (dimensionless)"
    legend_algebraic[
        48] = "Cai_bufc in component calcium_buffers_in_the_myoplasm (dimensionless)"
    legend_constants[40] = "CSQN_max in component calcium_buffers_in_the_JSR (mM)"
    legend_algebraic[
        50] = "Ca_JSR_bufc in component calcium_buffers_in_the_JSR (dimensionless)"
...
def initConsts():
    ...
    constants[44] = custom_piecewise(
        [equal(constants[4], 0.00000), 0.300000, True, 0.191000])
    constants[45] = custom_piecewise(
        [equal(constants[4], 0.00000), 2.50000, True, 2.00000])
    constants[46] = custom_piecewise(
        [equal(constants[4], 0.00000), 0.000850000, True, 0.00130000])
    constants[47] = custom_piecewise(
        [equal(constants[4], 0.00000), 0.00100000, True, 0.00000])
    constants[48] = custom_piecewise(
        [equal(constants[4], 0.00000), 1.30000, True, 0.750000])
    constants[49] = (1.00000/7.00000)*(exp(constants[13]/67.3000)-1.00000)
    constants[50] = custom_piecewise(
        [equal(constants[4], 0.00000), 1000.00, True, 1650.00])
    constants[51] = custom_piecewise(
        [equal(constants[4], 0.00000), 0.00450000, True, 0.00150000])
    constants[52] = custom_piecewise(
        [equal(constants[4], 0.00000), 0.000260000, True, 0.000170000])
...
def computeRates(voi, states, constants):
    ...
    rates[15] = custom_piecewise([less(states[14], 0.200000) & greater(
        states[14], 0.180000), 100.000*(1.00000-states[15])-0.500000*states[15], True, -0.500000*states[15]])
    rates[16] = custom_piecewise([less(states[14], 0.200000) & greater(
        states[14], 0.180000), 100.000*(1.00000-states[16])-0.500000*states[16], True, -0.0100000*states[16]])
    rates[18] = custom_piecewise([greater(1.00000/(1.00000+constants[30]/states[13]), constants[31]) & less(
        states[20], 0.370000) & less(states[16], 0.370000), 0.00000*50.0000*(1.00000-states[18]), True, -0.00000*0.500000*states[18]])
    rates[19] = custom_piecewise([greater(states[18], constants[32]) & less(
        states[19], constants[32]), 0.00000*50.0000*(1.00000-states[19]), True, -0.00000*0.500000*states[19]])
    rates[20] = custom_piecewise([greater(states[18], constants[32]) & less(
        states[20], constants[32]), 0.00000*50.0000*(1.00000-states[20]), True, -0.00000*0.0100000*states[20]])
    algebraic[1] = custom_piecewise([greater(fabs(states[0]+47.1300), 0.00100000), (0.320000*(
        states[0]+47.1300))/(1.00000-exp(-0.100000*(states[0]+47.1300))), True, 3.20000])
    ...
    algebraic[12] = custom_piecewise([less(states[0], -40.0000), 3.56000*exp(0.0790000*states[0])+310000.*exp(
        0.350000*states[0]), True, 1.00000/(0.130000*(1.00000+exp(-(states[0]+10.6600)/11.1000)))])
    rates[3] = algebraic[2]*(1.00000-states[3])-algebraic[12]*states[3]
    algebraic[3] = custom_piecewise([less(states[0], -40.0000), ((-127140.*exp(0.244000*states[0])-3.47400e-05 *
                                    exp(-0.0439100*states[0]))*(states[0]+37.7800))/(1.00000+exp(0.311000*(states[0]+79.2300))), True, 0.00000])
    algebraic[13] = custom_piecewise([less(states[0], -40.0000), (0.121200*exp(-0.0105200*states[0]))/(1.00000+exp(-0.137800*(
        states[0]+40.1400))), True, (0.300000*exp(-2.53500e-07*states[0]))/(1.00000+exp(-0.100000*(states[0]+32.0000)))])
    rates[4] = algebraic[3]*(1.00000-states[4])-algebraic[13]*states[4]
    algebraic[4] = (14.9859/(16.6813*(power(2.00000 * pi, 1.0/2)))) * \
        exp(-(power((states[0]-22.3600)/16.6813, 2.00000))/2.00000)
    algebraic[14] = 0.147100-(5.30000/(14.9300*(power(2.00000 * pi, 1.0/2)))) * \
        exp(-(power((states[0]-6.27440)/14.9300, 2.00000))/2.00000)
    rates[6] = algebraic[4]*(1.00000-states[6])-algebraic[14]*states[6]
    algebraic[5] = 0.00687200/(1.00000+exp((states[0]-6.15460)/6.12230))
    algebraic[15] = (0.0687000*exp(-0.108100*(states[0]+9.82550))+0.0112000) / \
        (1.00000+exp(-0.277900*(states[0]+9.82550)))+0.000547400
    rates[7] = algebraic[5]*(1.00000-states[7])-algebraic[15]*states[7]
    algebraic[6] = (0.526600*exp(-0.0166000*(states[0]-42.2912))) / \
        (1.00000+exp(-0.0943000*(states[0]-42.2912)))
    algebraic[16] = (5.18600e-05*states[0]+0.514900*exp(-0.134400 *
                     (states[0]-5.00270)))/(1.00000+exp(-0.134800*(states[0]-5.18600e-05)))
    rates[10] = algebraic[6]*(1.00000-states[10])-algebraic[16]*states[10]
    algebraic[7] = (5.61200e-05*states[0]+0.0721000*exp(-0.173000 *
                    (states[0]+34.2531)))/(1.00000+exp(-0.173200*(states[0]+34.2531)))
    algebraic[17] = (0.000121500*states[0]+0.0767000*exp(-1.66000e-09 *
                     (states[0]+34.0235)))/(1.00000+exp(-0.160400*(states[0]+34.0235)))
    rates[9] = algebraic[7]*(1.00000-states[9])-algebraic[17]*states[9]
    algebraic[8] = 0.00301300 / \
        (1.00000+exp((7.44540-(states[0]+10.0000))/14.3171))
    algebraic[18] = 0.00587000 / \
        (1.00000+exp((5.95000+states[0]+10.0000)/15.8200))
    rates[11] = algebraic[8]*(1.00000-states[11])-algebraic[18]*states[11]
    algebraic[9] = (0.00500000*exp(0.000526600*(states[0]+4.06700))
                    )/(1.00000+exp(-0.126200*(states[0]+4.06700)))
    algebraic[19] = (0.0160000*exp(0.00160000*(states[0]+65.6600))
                     )/(1.00000+exp(0.0783000*(states[0]+65.6600)))
    rates[12] = algebraic[9]*(1.00000-states[12])-algebraic[19]*states[12]
    algebraic[25] = log((0.0430000*constants[13]+constants[17]) /
                        (0.0430000*states[1]+states[8]))/constants[43]
    algebraic[26] = constants[44]*states[10] * \
        states[9]*(states[0]-algebraic[25])
    algebraic[30] = 1.00000/(1.00000+exp((states[0]+26.0000)/23.0000))
    algebraic[29] = log(constants[17]/states[8])/constants[43]
    algebraic[31] = constants[19]*states[12] * \
        algebraic[30]*(states[0]-algebraic[29])
    algebraic[27] = log((0.0183300*constants[13]+constants[17]) /
                        (0.0183300*states[1]+states[8]))/constants[43]
    algebraic[28] = constants[18] * \
        (power(states[11], 2.00000))*(states[0]-algebraic[27])
    algebraic[32] = log(constants[17]/states[8])/constants[43]
    algebraic[33] = 0.100000 / \
        (1.00000+exp(0.0600000*(states[0]-(algebraic[32]+200.000))))
    algebraic[34] = (3.00000*exp(0.000200000*(states[0]+100.000+-algebraic[32]))+1.00000*exp(
        0.100000*(states[0]-(10.0000+algebraic[32]))))/(1.00000+exp(-0.500000*(states[0]-algebraic[32])))
    algebraic[35] = algebraic[33]/(algebraic[33]+algebraic[34])
    algebraic[36] = constants[45]*algebraic[35]*(states[0]-algebraic[32])
    algebraic[39] = 1.00000/(1.00000+0.124500*exp(-0.100000*states[0] *
                             constants[43])+0.0365000*constants[49]*exp(-states[0]*constants[43]))
    algebraic[40] = (((constants[48]*algebraic[39]*1.00000)/(1.00000+power(
        constants[20]/states[1], 1.50000)))*constants[17])/(constants[17]+constants[21])
    algebraic[0] = floor(voi/constants[6])*constants[6]
    algebraic[10] = custom_piecewise([greater_equal(voi-algebraic[0], constants[5]) & less_equal(
        voi-algebraic[0], constants[5]+constants[7]), constants[8], True, 0.00000])
    rates[8] = (-1.00000*constants[3]*((algebraic[26]+algebraic[31]+algebraic[36] +
                algebraic[10]+algebraic[28])-2.00000*algebraic[40]))/(constants[54]*constants[2])
    algebraic[22] = log(constants[15]/states[5])/(2.00000*constants[43])
    algebraic[23] = constants[16]/(constants[16]+states[5])
    algebraic[24] = constants[14]*states[6]*states[7] * \
        algebraic[23]*(states[0]-algebraic[22])
    algebraic[41] = ((((((constants[50]*1.00000)/(power(constants[22], 3.00000)+power(constants[13], 3.00000)))*1.00000)/(constants[23]+constants[15]))*1.00000)/(1.00000+constants[24]*exp((constants[25]-1.00000)
                     * states[0]*constants[43])))*(exp(constants[25]*states[0]*constants[43])*(power(states[1], 3.00000))*constants[15]-exp((constants[25]-1.00000)*states[0]*constants[43])*(power(constants[13], 3.00000))*states[5])
    algebraic[37] = constants[46]*(states[0]-algebraic[22])
    rates[17] = custom_piecewise([greater(states[14], 0.200000), (-constants[3]*((algebraic[24]-algebraic[41])+algebraic[37]))/(
        2.00000*constants[54]*constants[2]), greater(states[15], 0.0100000) & less_equal(states[14], 0.200000), 0.00000, True, -0.500000*states[17]])
    algebraic[20] = log(constants[13]/states[1])/constants[43]
    algebraic[21] = constants[12] * \
        (power(states[2], 3.00000))*states[3] * \
        states[4]*(states[0]-algebraic[20])
    algebraic[38] = constants[47]*(states[0]-algebraic[20])
    rates[1] = (-1.00000*constants[3]*(algebraic[21]+algebraic[38]+algebraic[41]
                * 3.00000+algebraic[40]*3.00000))/(constants[54]*constants[2])
    algebraic[42] = algebraic[21]+algebraic[24]+algebraic[26]+algebraic[31]+algebraic[28] + \
        algebraic[36]+algebraic[41]+algebraic[40] + \
        algebraic[38]+algebraic[37]+algebraic[10]
    algebraic[44] = custom_piecewise([equal(constants[9], 1.00000) & (less_equal(voi, 500.000) | greater(voi, 800.000)), (constants[10]-states[0])/1.00000, equal(
        constants[9], 1.00000) & greater(voi, 500.000) & less_equal(voi, 800.000), (constants[11]-states[0])/1.00000, True, -1.00000*algebraic[42]])
    rates[0] = algebraic[44]
    rates[14] = custom_piecewise([greater(algebraic[44], 150.000), 100.000*(
        1.00000-states[14])-0.500000*states[14], True, -0.500000*states[14]])
    algebraic[43] = custom_piecewise([greater(states[17], constants[29]), ((1.00000*constants[26]*(states[17]-constants[29]))/((constants[28]+states[17])-constants[29]))*(
        1.00000-states[15])*states[15], less_equal(states[17], constants[29]) & greater(states[19], 0.00000), 0.00000*constants[27]*(1.00000-states[19])*states[19], True, 0.00000])
    ...
    algebraic[48] = 1.00000/(1.00000+constants[39]*((constants[38]*constants[36])/(power(
        constants[36]+states[5], 2.00000))+(constants[37]*constants[35])/(power(constants[35]+states[5], 2.00000))))
    rates[5] = algebraic[48]*((-constants[3]*((algebraic[24]-2.00000*algebraic[41])+algebraic[37]))/(2.00000*constants[54]
                              * constants[2])+(algebraic[45]*constants[56])/constants[54]+((algebraic[47]-algebraic[46])*constants[55])/constants[54])
    algebraic[49] = (1.00000*(states[21]-states[13]))/constants[34]
    rates[21] = -1.00000*((algebraic[47]+(constants[56] /
                          constants[55])*algebraic[49])-algebraic[46])
    algebraic[50] = 1.00000/(1.00000+(constants[40]*constants[30]) /
                             (power(constants[30]+states[13], 2.00000)))
